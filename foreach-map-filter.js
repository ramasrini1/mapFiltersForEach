...
function addKeyAndValue(arr,key,value){
    ...
    let i = 0;
    arr.forEach(function(val){
        updatedArray[i++] = {name: val.name, 'title': 'instructor'};
    });
    return updatedArray;
}
...
function doubleValuesWithMap(arr) {
    let doubleArray = arr.map(function(values){
        return values*2;
    });
    return doubleArray;
}
...
function valTimesIndex(arr){
    let valTimesIndex = arr.map(function(values, index){
        return values * index;
    });
...
and returns a new array with the value of the key with a name of 
"first" and the value of a key with the name of  "last" in each object, 
...
function extractFullName(arr){
    let nameArray = [];
    arr.map(function(obj){
        nameArray.push(obj['first'] + " " + obj['last']);
    });
    return nameArray;  
}
...
returns the first element in the array that has the same value 
...
a key, and some value to search for and returns the first 
...
function removeVowels(str) {
    ...
    lowerStrArr.filter(function( char ){
        if ( vowels.indexOf(char) === -1){
            nonVowelsArr.push(char);
        }   
    });
    //convert array of chars to string
    return nonVowelsArr.join("");
}
